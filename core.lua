  ...
  -- v13 migration: dock to left edge
  if DB.pos and DB.pos.point == 'LEFT' and DB.pos.relPoint == 'LEFT' and DB.pos.x == 20 then DB.pos.x = 0 end

  -- Migrate old default CENTER position to LEFT (Galaxy layout)
  if DB.pos and DB.pos.point == 'CENTER' and DB.pos.relPoint == 'CENTER' and (DB.pos.x or 0) == 0 and (DB.pos.y or 0) == 0 then
    DB.pos.point, DB.pos.relPoint, DB.pos.x, DB.pos.y = 'LEFT', 'LEFT', 20, 0
  end
  ...
  manager:SetSize(200,390)
    -- position applied from SavedVariables
  ...
  manager:SetShown(DB.enabled)

  manager:SetScale(DB.scale or 1.0)
  ...
  -- Slash: /rrm lock|unlock|toggle|reset
  SLASH_ROTHRAIDMANAGER1 = "/rrm"
  SlashCmdList.ROTHRAIDMANAGER = function(msg)
    msg = (msg or ""):lower():gsub("^%s+",""):gsub("%s+$","")
    if msg == "unlock" then
      DB.locked = false
      manager:EnableMouse(true)
      print("|cff00ff00RothRaidManager|r unlocked (ALT-drag).")
    ...
    elseif msg == "reset" then
      ...
      manager:ClearAllPoints()
      manager:SetPoint(DB.pos.point, UIParent, DB.pos.relPoint, DB.pos.x, DB.pos.y)
      print("|cff00ff00RothRaidManager|r position reset.")
    else
      print("|cff00ff00RothRaidManager|r: /rrm lock | unlock | toggle | reset")
    end
  end